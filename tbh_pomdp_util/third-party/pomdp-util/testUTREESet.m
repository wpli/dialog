...
% runs utree... the parameters were played around with/optimized for the tiger and 4x3
% gridworlds; beware there are many hard-coded bits in here!
...
while( keep_running )
    ...
    % update the transition and reward matrices -- finale! note that
    % the transition are definied (s,s',a) for ease of value iteration
    % in the next step (*not* how the pomdp transitions are defined!)
...
% finale! for now the depth can only be 1 or 2
if use_action_index
...
% finale! for now we just try two search depths: one/two items in the past
...
for action_index = 1:length( unique_action_set )
    ...
    % finale! arbitrary cut-off for fringe!
...
% finale! want to adjust only the new nodes!
