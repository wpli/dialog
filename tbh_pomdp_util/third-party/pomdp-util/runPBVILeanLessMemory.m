function [TAO vL backupStats] = runPBVILeanLessMemory(problem,S,h,TAO,epsilon)
% function [TAO vL backupStats] = runPBVILean(problem,S,h,TAO,epsilon)
...
if issparse( problem.reward )
    for action = 1:problem.nrActions
        Q{ action } = sparse( repmat( problem.reward( : , action )' , [ beliefCount 1 ] ) );
    end
else
...
% computes the Q vectors for the next backup
    ...
    for action = 1:problem.nrActions
        ...
        gammaAB = sparse( repmat( problem.reward(:,action)  ,1,size(S,1)  ) );
        for obs = 1:problem.nrObservations
...
function backupStats = findBestQLean(problem, S,Q)
    ...
    if iscell( Q )
        ...
        v = sparse( size( Q{ 1 } , 1 ) , size( Q{ 1 } , 2 ) );
        for a = 1:problem.nrActions % select the best actions
            v = v + Q{ a } .* sparse( repmat( transpose(action == a), 1, problem.nrStates ) );
        end
        a = action';
        clear mask;
        ...
        % finale! orig! v = squeeze(sum( Q.*mask,3));
        v = sum( Q .* mask , 3 );
        a = action';
        clear mask;
