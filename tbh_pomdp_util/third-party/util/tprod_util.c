...
$Id: tprod.c,v 1.53 2007-09-07 13:56:14 jdrf Exp $
 ...
 *        the output matrix.
 *-------------------------------------------------------------------------*/
int* compx2yIdx(MxInfo xinfo, int xnidx, int *xidx,
					 MxInfo yinfo, int ynidx, int *yidx,
					 int *znd, int *maccnd, int *seqnd){
  *znd=0; *maccnd=0; *seqnd=0; 
  ...
  char msgTxt[256];
  int *x2yIdx=(int*)CALLOC(xnidx+ynidx,sizeof(int));  	 
  int i,j;
  ...
  *znd=MAX((int)xidx[0],0);
  for( i=1; i<xnidx; i++){
	 if ( xidx[i] < 0 ) {
		for( j=0; j<i; j++)
		  if(xidx[i]==xidx[j]) ERROR("tprod: Duplicate x-dim label");
	 } else if ( xidx[i] > 0 ) {
  ...
  *znd=MAX(*znd,(int)yidx[0]); 
  for( i=1; i<ynidx; i++){
	 if ( yidx[i] < 0 ) {
		for( j=0; j<i; j++)
		  if(yidx[i]==yidx[j]) ERROR("tprod: Duplicate y-dim label");
...
void initrestmaccmxInfo(int znd,
  ...
  if( xrestinfo->nd < znd || yrestinfo->nd < znd ) 
	 ERROR("mxInfo structures size too small");
  ...
  for( i=0; i<xnidx; i++ ) {
	 ...
	 if ( x2yIdx[i] < 0 ) { /* accumulated dimension */
		...
		if ( sz(*xmaccinfo,maccnd) != sz(*ymaccinfo,maccnd) ) { 
		  ERROR("Accumulated dimensions sizes must match!");		  
		}
