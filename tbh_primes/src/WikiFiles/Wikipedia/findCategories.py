...
import pickle
s="speak the text on the screen"
...
#	print
	...
	for b in range(len(twowords)):
		for c in range(len(sentencewords1)):
			fixedword=sentencewords1[c]
			if fixedword[len(fixedword)-2:]=="'s":
				fixedword=fixedword[:(len(fixedword)-2)]
			parts=twowords[b].split("_")
			for d in range(len(parts)):
				if parts[d]==fixedword:
					tempscore=tempscore+(1000/len(parts))
