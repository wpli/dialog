...
class tbh_dialog_manager_t:
    ...
    def _process_environmental_control_event( self, ec_action ):
        for handler in self._environmental_control_handler_list:
            t = threading.Thread( target=functools.partial( handler , ec_action ) )
            t.start()
   ...
   # Adds more handlers for when nbest stuff needs processing
    ...
    def add_environmental_control_handler( self, handler ):
        self._environmental_control_handler_list.append( handler )
