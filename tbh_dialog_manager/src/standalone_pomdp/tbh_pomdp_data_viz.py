...
class tbh_pomdp_data_viz_t(QMainWindow):
    ...
    def on_update_button( self ):
        ...
        if self.dialog_type == "all_incorrect":
            ...
            self.draw_from_data( self.organized_dialog_entries.incorrect_dialog_list[dialog_number]['turn_list'][turn_number]['pomdp_dict']['belief_for_policy'] )
        
            print_turn_data_information( self.organized_dialog_entries.incorrect_dialog_list, dialog_number, turn_number )

        elif self.dialog_type == "all_correct":
            dialog_number = int( self.dialog_number_textbox.text() )
            turn_number = int( self.turn_number_textbox.text() )
            self.draw_from_data( self.organized_dialog_entries.correct_dialog_list[dialog_number]['turn_list'][turn_number]['pomdp_dict']['belief_for_policy'] )
            print_turn_data_information( self.organized_dialog_entries.correct_dialog_list, dialog_number, turn_number )

        elif self.dialog_type == "four_long":
            dialog_number = int( self.dialog_number_textbox.text() )
            turn_number = int( self.turn_number_textbox.text() )
            self.draw_from_data( self.organized_dialog_entries.four_long_dialog_list[dialog_number]['turn_list'][turn_number]['pomdp_dict']['belief_for_policy'] )
            print_turn_data_information( self.organized_dialog_entries.four_long_dialog_list, dialog_number, turn_number )
    ...
    def on_draw(self):
        ...
        self.axes.set_xbound( lower=0, upper=65 )
    ...
    def annotate_axes( self ):
        ...
        if len( key_states_list ) <= len( annotation_letters ):
            labeled_states_list = key_states_list[:]

        else: 
            key_states_list.sort()
            labeled_states_list = []
            for i in range( len( annotation_letters ) ):
                labeled_states_list.append( key_states_list[-1-i] )
    
        #print "KEY STATES", key_states_list
        
        state_legend_list = []

        for index, entry in enumerate( labeled_states_list ):
           #print index
           annotation_string = annotation_letters[ index ]


           legend_string = "%s: %s" % ( annotation_string, \
    ...
    def reset_belief_handler( self ):
        self.axes.clear()        
        #self.belief_vector = belief
        self.draw_from_data( numpy.zeros( 62 ) )
        self.status_text.setText( "Start of new dialog." )
...
def get_state_code_list( state_file ):
    ...
    for line in f.readlines():
        ...
        state_code_list.append( [ split_line[0], split_line[1:] ] )    
    f.close()
    #print state_code_list
    return state_code_list
    

def print_turn_data_information( dialog_list, dialog_number, turn_number ):
    ...
    print "Agent's Utterance: %s, %s, %.5f" % (
        turn['turn_dict']['sample_utterance'], \
        turn['turn_dict']['sample_utterance_info']['orthography'], \
        turn['turn_dict']['sample_utterance_info']['confidence_score'] )
        

    print "Machine Action:", turn['pomdp_dict']['new_machine_action']
