...
DEFAULT_CONTACTS_FILE = "null"

##
# Description:
# Arduino class
class tbh_environmental_control_t():
    # Constructor for the Arduino instance
    def __init__( self ):
    ...
    def lampOn( self ):
        print"Lamp on"
        self.arduino.write("1") # This is the on command     
        time.sleep(1.5)
        self.arduino.write("N") # N is the off command
    ...
    def execute_action_handler( self, action_code ):
        print "IN ENVIRONMENTAL CONTROL MODULE"
        base_code = action_code[0]
        device_action = action_code[1][0]
        if device_action == ( "radio_on", "let_there_be_light" ):
            self.radioOn()
        elif device_action in ( "radio_off" ):
            self.radioOff()
        elif device_action == "fan_on":
            self.fanOn()
        elif device_action == "fan_off":
            self.fanOff()
        else:
            pass
...
if __name__ == "__main__":
    environmental_control = tbh_environmental_control_t()
    environmental_control.fanOn()

    time.sleep(8)
    environmental_control.fanOff()
